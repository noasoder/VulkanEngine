...
void VulkanManager::DrawFrame(float DeltaTime) {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        throw std::runtime_error("failed to acuire swap chain image");
    }
...
void VulkanManager::CreateGraphicsPipeline() {
    auto vertShaderCode = readFile("../bin/Assets/Shaders/vert.spv");
    auto fragShaderCode = readFile("../bin/Assets/Shaders/frag.spv");
...
bool VulkanManager::CheckDeviceExtensionSupport(VkPhysicalDevice device) {
    ...
    for (const auto& extension : requiredExtensions) {
        std::cout << std::endl << "required extension: " << extension << std::endl;
    }
    ...
    for (const auto& extension : requiredExtensions) {
        std::cout << "required extension not supported: " << extension << std::endl;
    }
...
void VulkanManager::CreateSwapChain() {
    ...
    QueueFamilyIndices indices = FindQueueFamilies(m_physicalDevice);
    uint32_t queryFamilyIndices[] = { indices.graphicsFamily.value(), indices.presentFamily.value() };

    if (indices.graphicsFamily != indices.presentFamily) {
        createInfo.imageSharingMode = VK_SHARING_MODE_CONCURRENT;
        createInfo.queueFamilyIndexCount = 2;
        createInfo.pQueueFamilyIndices = queryFamilyIndices;
    }
    else {
